...
def get_potential_xvg_files_to_csvfiles(MD_path):
    ...
    for padded_num in (f"{i:03}" for i in range(1, pv.PROTEIN.dataset_length+1)): #pv.PROTEIN.dataset_length+1
        print(padded_num)
        combined_path = MD_path / padded_num
        if combined_path.exists() and combined_path.is_dir():
            os.chdir(MD_path / f'{padded_num}')
            potential_xvgfile = combined_path / f'{padded_num}_lig_pot_energy.xvg'
            # If the file already exists, modify the filename to avoid overwriting
            if potential_xvgfile.exists():
                column_names = ['picoseconds']
                with open(potential_xvgfile, 'r') as file:
        ...
        filtered_dataframe.insert(0, 'mol_id', padded_num)
    ...
    all_data.to_csv(pv.energyfolder_path_ / 'potential.csv', index=False)
...
# def get_potential_xvg_files_to_csvfiles(rmsd_xvgfolder_path):
...
#         if xvg_file.name.startswith('#'):
#             print(f"Skipping {xvg_file.name} (starts with #)")
#             continue
...
#     #MDfeatures_allmol_csvfile_ = generate_new_csv_filename(sasa_xvgfolder_path, public_variables.MDfeatures_allmol_csvfile)
#     all_data.to_csv(pv.energyfolder_path_ / 'rmsd.csv', index=False)
#     return all_data

def main(MDsimulations_path = pv.MDsimulations_path_):
    potential_df = get_potential_xvg_files_to_csvfiles(MDsimulations_path)
    return potential_df
