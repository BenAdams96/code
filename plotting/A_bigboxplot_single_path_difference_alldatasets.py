...
def boxplots_compare_groups(path, csv_filename, modelresults_dict, combined):
    """
    """
    print(path)
    print(csv_filename)
    ...
    labels = {
        ...
        f'{pv.dfs_reduced_and_MD_path_.name}__{pv.dfs_2D_path.name}': 'difference red_MD - 2D_ECFP',
    ...
    # Save the plot
    if combined:
        plt.title(f"{pv.ML_MODEL}: All datasets combined: Barplot - Difference between groups")
    else:
        plt.title(f"{pv.ML_MODEL} ; {pv.PROTEIN}: Barplot - Difference between groups")
    
    plt.xlabel("Conformations Group")
    # Save the plot
    if combined:
        plot_file_path = save_plot_folder / f'{pv.ML_MODEL}_DIFF_{len(modelresults_dict)}_Combined.png'
    else:
...
# def average_dict_results(all_modelresults_dict_temp):
#     output_dict = {}
#     for model_name, model_data in all_modelresults_dict_temp.items():
#         for group, subgroups in model_data.items():

#         output_dict[model_name] = []
#     return

def average_dict_results(all_modelresults_list):
    output_dict = {}

    # Iterate over the list of model results from each protein
    for modelresults_dict_temp in all_modelresults_list:
        for model_name, model_data in modelresults_dict_temp.items(): #modelname = 'ModelResults_R2_WHIM.csv'
            if model_name not in output_dict:
                output_dict[model_name] = {}

            for group, subgroups in model_data.items(): #group = 'reduced_t0.85_MD__2D', subgroup is dictionary with 0ns etc
                if group not in output_dict[model_name]:
                    output_dict[model_name][group] = {}

                # Now iterate over the time points (e.g., '0ns', '2ns', etc.)
                for time_point, values in subgroups.items():
                    # Average the list of values for the current time_point across all proteins
                    if time_point not in output_dict[model_name][group]:
                        output_dict[model_name][group][time_point] = []

                    # Append the current values (the list of 10 values) to the list
                    output_dict[model_name][group][time_point].append(values)

    # Now average the results for each time point across all proteins
    for model_name, model_data in output_dict.items():
        for group, subgroups in model_data.items():
            for time_point, all_values in subgroups.items():
                # Convert list of lists into a numpy array, then compute the mean across the lists
                all_values_array = np.array(all_values)
                output_dict[model_name][group][time_point] = np.mean(all_values_array, axis=0)
...
def main(dfs_paths = [pv.dfs_descriptors_only_path_]):
    ...
    print(dfs_paths)
    modelresults_dict_temp = {}
    for dfs_path1, dfs_path2_min, list_to_include in dfs_paths:
        modelresults_dict_temp = nested_data_dict(dfs_path1, dfs_path2_min,modelresults_dict_temp, list_to_include)
        print(modelresults_dict_temp)

    for csvfile_name, modelresults_dict in modelresults_dict_temp.items(): #loop over k10_r2 etc.
        # boxplots_compare_individuals(master_folder, csvfile_name, modelresults_dict)
        print(csvfile_name) #ModelResults_R2_WHIM.csv
        boxplots_compare_groups(path=pv.dataframes_master_, csv_filename=csvfile_name, modelresults_dict=modelresults_dict, combined=False)

    
    return modelresults_dict_temp
...
if __name__ == "__main__":
    ...
    for model in Model_classic:
        all_modelresults_list = []
        for protein in list(DatasetProtein)[:4]:
            print(protein)
            pv.update_config(protein_=protein)
            dfs_paths = []
            pv.update_config(model_=model, descriptor_=Descriptor.WHIM, hyperparameter_set='big')
            ...
            modelresults_dict_temp = main(dfs_paths)


            
            all_modelresults_list.append(modelresults_dict_temp)
        print('test')
        output_dict = average_dict_results(all_modelresults_list)
        for csvfile_name, modelresults_dict in output_dict.items(): #loop over k10_r2 etc.
            # boxplots_compare_individuals(master_folder, csvfile_name, modelresults_dict)
            print(csvfile_name) #ModelResults_R2_WHIM.csv
            boxplots_compare_groups(path=pv.dataframes_master_, csv_filename=csvfile_name, modelresults_dict=modelresults_dict, combined=True)
