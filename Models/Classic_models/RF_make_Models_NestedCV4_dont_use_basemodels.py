...
def nested_cross_validation(name, df, dfs_path, outer_folds=10, inner_folds=5):
    ...
    for outer_fold, (train_idx, test_idx) in enumerate(outer_cv.split(X=grouped_df, y=grouped_binned_y, groups=unique_mol_ids)):
        ...
        grid_search = hyperparameter_tuning(model_instance, X, y, pv.ML_MODEL.hyperparameter_grid, cv=custom_inner_splits, scoring='neg_root_mean_squared_error')
        ...
        all_idx_ypredicted_pki_series = pd.concat([all_idx_ypredicted_pki_series, pd.Series(y_predicted, index=y_test.index)]).sort_index()
    ...
    save_path = dfs_path / pv.Modelresults_folder_ / 'true_vs_prediction' / f'{name}.csv'

    save_path.parent.mkdir(parents=True, exist_ok=True)
    df_ytrue_ypred_pki.to_csv(save_path, index=False)

    #average the results (for when multiple conformations)
    average_results = df_ytrue_ypred_pki.groupby('mol_id').mean()
    r2_score_molecule_level = r2_score(average_results['True_pKi'], average_results['Predicted_pKi'])
    mse_molecule_level = mean_squared_error(average_results['True_pKi'], average_results['Predicted_pKi'])
    mae_molecule_level = mean_absolute_error(average_results['True_pKi'], average_results['Predicted_pKi'])
    print(fold_results)
...
def main(dfs_path = pv.dfs_descriptors_only_path_):
    ...
    dfs_in_dic = csv_to_dictionary.csvfiles_to_dic_include(dfs_path, include_files=['0ns.csv','1ns.csv','2ns.csv','3ns.csv','4ns.csv','5ns.csv','6ns.csv','7ns.csv','8ns.csv','9ns.csv','10ns.csv','conformations_10.csv','conformations_20.csv','minimized_conformations_10.csv'])
    # dfs_in_dic = csv_to_dictionary.csvfiles_to_dic_include(dfs_path, include_files=['conformations_10.csv'])
    ...
    csv_filename = f'ModelResults_Ko{outer_folds}_Ki{inner_folds}_{scoring}_{pv.DESCRIPTOR}.csv'
