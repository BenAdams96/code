...
from global_files import csv_to_dictionary, public_functions, public_variables
...
def nested_cross_validation(name, df, dfs_path, outer_folds=10, inner_folds=5):
    ...
    for outer_fold, (train_idx, test_idx) in enumerate(outer_cv.split(grouped_df, grouped_binned_y, groups=groups_molid)):
        ...
        svm_model = SupportVectorMachineRegressor()
        svm_model.model.random_state = 42
        
        grid_search = svm_model.hyperparameter_tuning(X, y,public_variables.hyperparameter_grid_SVM,cv=custom_inner_splits,scoring_='r2')
        df_results = pd.DataFrame(grid_search.cv_results_)
        # print(df_results)
        #build a "final" model for this train test split.
        svm_model = SupportVectorMachineRegressor(kernel=grid_search.best_params_['kernel'],
        ...
        r2_score = svm_model.model.score(X_test, y_test)
    ...
    save_path = dfs_path / 'ModelResults_SVM' / 'true_vs_prediction' / f'{name}.csv'
...
if __name__ == "__main__":
    #NOTE: dont forget to change public variables!!!!
