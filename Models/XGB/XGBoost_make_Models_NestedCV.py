...
def nested_cross_validation(name, df, dfs_path, outer_folds=10, inner_folds=5):
    ...
    for outer_fold, (train_idx, test_idx) in enumerate(outer_cv.split(grouped_df, grouped_binned_y, groups=groups_molid)):
        ...
        xgboost_model.model.random_state = 42
        start_time1 = time.time()
        print('start timer hp')
        grid_search = xgboost_model.hyperparameter_tuning(X, y,public_variables.hyperparameter_grid_XGB,cv=custom_inner_splits,scoring_='r2')
        end_time1 = time.time()
        elapsed_time1 = end_time1 - start_time1
        print(f"Elapsed time for HP: {elapsed_time1:.2f} seconds")
        df_results = pd.DataFrame(grid_search.cv_results_)
        # print(df_results)
        #build a "final" model for this train test split.
        xgboost_model.model.set_params(
        ...
        print('start timer fit')
        xgboost_model.model.random_state = 42
        xgboost_model.model.fit(X_train, y_train)
        y_pred = xgboost_model.model.predict(X_test)
        r2_score = xgboost_model.model.score(X_test, y_test)
        ...
        print(f"Elapsed time for fit predict score: {elapsed_time:.2f} seconds")
    ...
    save_path = dfs_path / 'ModelResults_XGB' / 'true_vs_prediction' / f'{name}.csv'
