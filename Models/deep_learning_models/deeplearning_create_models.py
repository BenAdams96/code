...
def random_outer_splitting(outer_fold_number, grouped_df, grouped_binned_y, unique_mol_ids):
    ...
    val_train_mol_ids, test_mol_ids = train_test_split(unique_mol_ids, test_size=0.2, stratify=grouped_binned_y[unique_mol_ids], random_state=random_state_value)
...
def kfold_outer_splitting(outer_fold_number, outer_folds, grouped_df,grouped_binned_y, unique_mol_ids):
    ...
    train_outer_mol_ids = sorted(set(train_mol_ids) - set(val_outer_mol_ids))
    # print('test')
    # print(sorted(test_mol_ids))
...
def deeplearning_function(name, df, dfs_path, random_splitting = False):
    ...
    for outer_fold_idx in range(outer_folds):
        ...
        if pv.ML_MODEL == Model_deep.LSTM:
            ...
            final_model, train_outer_losses, val_outer_losses = build_DNN_model(train_outer_loader, val_outer_loader, input_size, num_of_conformations, nums_of_epochs, patience, hyperparameter_set)
...
if __name__ == "__main__":
    ...
    main(pv.dfs_reduced_path_, random_splitting = False, include_files = ['0ns.csv','2ns.csv','4ns.csv','6ns.csv','8ns.csv','10ns.csv','conformations_10.csv','conformations_20.csv','minimized_conformations_10.csv'])
