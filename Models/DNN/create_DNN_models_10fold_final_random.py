...
def DNN_function(name, df, dfs_path):
    print('nested cross validation for: ', name)
    ...
    hyperparameter_grid = [
        ...
        # {"learning_rate": 0.0005, "hidden_layers": [64, 32, 16],"dropout_rate": 0.1}, #increase dropout rate to 0.2 as well because small dataset
        # {"learning_rate": 0.0005, "hidden_layers": [64, 32],"dropout_rate": 0.1},
        # {"learning_rate": 0.0005, "hidden_layers": [128, 64, 32],"dropout_rate": 0.1},
        # {"learning_rate": 0.0005, "hidden_layers": [128, 64],"dropout_rate": 0.1},
        # {"learning_rate": 0.0005, "hidden_layers": [64, 32, 16],"dropout_rate": 0.1},
        # {"learning_rate": 0.0005, "hidden_layers": [64, 32],"dropout_rate": 0.1},
        # {"learning_rate": 0.0005, "hidden_layers": [128, 64, 32],"dropout_rate": 0.1},
        # {"learning_rate": 0.0005, "hidden_layers": [128, 64],"dropout_rate": 0.1},
        ...
        # {"learning_rate": 0.001, "hidden_layers": [64, 32, 16],"dropout_rate": 0.2}, #increase dropout rate to 0.2 as well because small dataset
        # {"learning_rate": 0.001, "hidden_layers": [64, 32],"dropout_rate": 0.2},
        # {"learning_rate": 0.001, "hidden_layers": [128, 64, 32],"dropout_rate": 0.2},
        # {"learning_rate": 0.001, "hidden_layers": [128, 64],"dropout_rate": 0.2},
        # {"learning_rate": 0.001, "hidden_layers": [64, 32, 16],"dropout_rate": 0.2},
        # {"learning_rate": 0.001, "hidden_layers": [64, 32],"dropout_rate": 0.2},
        # {"learning_rate": 0.001, "hidden_layers": [128, 64, 32],"dropout_rate": 0.2},
        # {"learning_rate": 0.001, "hidden_layers": [128, 64],"dropout_rate": 0.2},
        # {"learning_rate": 0.001, "hidden_layers": [256, 128, 64],"dropout_rate": 0.2},
        # {"learning_rate": 0.001, "hidden_layers": [256, 128],"dropout_rate": 0.2},
    ]
    fold_results = {
        'R2': [],
        'MSE': [],
    ...
    all_mol_ids = list(grouped_df.index)
    ...
    for outer_fold_number in range(outer_folds):
        ...
        train_mol_ids, val_test_mol_ids = train_test_split(all_mol_ids, test_size=0.2, stratify=grouped_binned_y[all_mol_ids], random_state=random_state_value)
        
        # Step 2: Split the 30% into 10% validation and 20% test
        val_outer_mol_ids, test_mol_ids = train_test_split(val_test_mol_ids, test_size=3/4, stratify=grouped_binned_y[val_test_mol_ids], random_state=random_state_value)

        train_idx_all = df[df['mol_id'].isin(train_mol_ids)].index
        validation_outer_idx_all = df[df['mol_id'].isin(val_outer_mol_ids)].index
        ...
        for inner_fold, (train_inner_idx, validation_inner_idx) in enumerate(inner_cv.split(grouped_X_train, grouped_binned_y_train, groups=grouped_X_train.index)):
            ...
            y_validation_inner_grouped = y_validation_inner.values.reshape(-1, num_of_conformations)  # Shape: (num_molecules, 10)
            ...
            for param_idx, params in enumerate(hyperparameter_grid):
                model = FullyConnectedDNN(input_size=X_train_inner.shape[1], hidden_layers=params["hidden_layers"],dropout_rate=params["dropout_rate"])
                ...
                train_inner_losses, val_inner_losses = model.train_with_validation(
                    train_inner_loader, val_inner_loader, num_epochs=1000, num_of_conformations=num_of_conformations, optimizer=optimizer,
        ...
        with open(save_path / f"{name}_5loss_curves_of_best_inner_of_outer_{outer_fold_number}_random.json", "w") as f:
        ...
        final_model = FullyConnectedDNN(input_size=X_train.shape[1], hidden_layers=best_params["hidden_layers"],dropout_rate=best_params["dropout_rate"])
        ...
        train_outer_losses, val_outer_losses = final_model.train_with_validation(
                train_outer_loader, val_outer_loader, num_epochs=epochs, num_of_conformations=num_of_conformations, optimizer=optimizer,
                criterion=criterion, device="cuda" if torch.cuda.is_available() else "cpu", patience=250
            )
        ...
        final_model.eval()
        # Reshape X_test and y_test for conformity with the number of conformations
        ...
        y_test_grouped = y_test.values.reshape(-1, num_of_conformations)
        ...
        with torch.no_grad():
            for inputs, targets in test_loader:
                ...
                targets = targets.view(-1, num_of_conformations).mean(dim=1)
            ...
            mol_ids.extend(df.loc[test_idx_all, 'mol_id'].unique())  # Extract mol_id corresponding to test set
    ...
    with open(save_path / f'{name}_loss_curves_all_outer_folds_random.json', 'w') as f:
    ...
    save_path.mkdir(parents = True, exist_ok=True)
    df_true_predicted.to_csv(save_path / f'{name}_true_predicted_random.csv', index=False)
...
def save_fold_results(results, metric, ModelResults, Modelresults_path):
    ...
    csv_filename_temp = f'ModelResults_random_{metric}_{pv.DESCRIPTOR}_temp.csv'
    pd.DataFrame(ModelResults[metric]).to_csv(Modelresults_path / csv_filename_temp, index=False)

    csv_filename = f'ModelResults_random_{metric}_{pv.DESCRIPTOR}.csv'
...
def main(dfs_path = pv.dfs_descriptors_only_path_, include_files = []):
    ...
    if not include_files:
        include_files = ['0ns.csv','1ns.csv','conformations_10.csv']
