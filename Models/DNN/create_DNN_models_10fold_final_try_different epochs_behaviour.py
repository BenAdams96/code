...
def DNN_function(name, df, dfs_path):
    ...
    if name.startswith('conformations'):
        num_of_conformations = int(re.search(r"_(\d+)", name).group(1))
    else:
        num_of_conformations = 1
    ...
    for outer_fold_number, (train_idx, test_idx) in enumerate(outer_cv.split(X=grouped_df, y=grouped_binned_y, groups=unique_mol_ids)):
        ...
        for inner_fold, (train_inner_idx, validation_inner_idx) in enumerate(inner_cv.split(grouped_X_train, grouped_binned_y_train, groups=grouped_X_train.index)):
            ...
            for param_idx, params in enumerate(hyperparameter_grid):
                ...
                optimizer1 = torch.optim.Adam(model.parameters(), lr=params["learning_rate"])
                criterion1 = torch.nn.MSELoss()
                
                # Train with validation to tune hyperparameters
                train_inner_losses, val_inner_losses = model.train_with_validation(
                    train_inner_loader, val_inner_loader, num_epochs=100, num_of_conformations=num_of_conformations, optimizer=optimizer1,
                    criterion=criterion1, device="cuda" if torch.cuda.is_available() else "cpu", patience=100
                )
        ...
        #         criterion=criterion, device="cuda" if torch.cuda.is_available() else "cpu", patience=200
        #     )

        outer_performance = []
        num_of_epochs = [200,600,1000,1400]
        for x in num_of_epochs:
            print(x)
            final_model = FullyConnectedDNN(input_size=X_train.shape[1], hidden_layers=best_params["hidden_layers"],dropout_rate=best_params["dropout_rate"])
            optimizer = torch.optim.Adam(final_model.parameters(), lr=best_params["learning_rate"])
            criterion = torch.nn.MSELoss()

            train_outer_losses, val_outer_losses = final_model.train_without_validation(
                train_outer_loader, val_outer_loader, num_epochs=x, num_of_conformations=num_of_conformations, optimizer=optimizer,
                criterion=criterion, device="cuda" if torch.cuda.is_available() else "cpu", patience=150)
            ...
            outer_performance.append(r2_value)
            
        print(outer_performance)
