...
def LSTM_function(name, df, dfs_path):
    ...
    for outer_fold_number in range(outer_folds):
        ...
        for inner_fold, (train_inner_idx, validation_inner_idx) in enumerate(inner_cv.split(grouped_X_train, grouped_binned_y_train, groups=grouped_X_train.index)):
            ...
            y_train_inner_grouped = y_train_inner.values.reshape(-1, num_of_conformations).mean(axis=1, keepdims=True)  # Shape: (num_molecules, 10)
            ...
            y_validation_inner_grouped = y_validation_inner.values.reshape(-1, num_of_conformations).mean(axis=1, keepdims=True)
        ...
        y_train_outer_grouped = y_train.values.reshape(-1, num_of_conformations).mean(axis=1, keepdims=True)  # Shape: (num_molecules, 10)
        ...
        y_validation_outer_grouped = y_validation_outer.values.reshape(-1, num_of_conformations).mean(axis=1, keepdims=True)  # Shape: (num_molecules, 10)
