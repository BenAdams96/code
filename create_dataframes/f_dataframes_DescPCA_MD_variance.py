...
from create_dataframes import d_dataframes_MD_only, e_dataframes_DescPCA, e_dataframes_DescPCA_variance
...
def main(savefolder_name = pv.dfs_dPCA_var_MD_path_, include=[], variance=0.98, to_keep = ['SASA','num of H-bonds','H-bonds within 0.35A', 'Total dipole moment', 'Ligand Bond energy', 'Urey-Bradley energy', 'Torsional energy', 'Coul-SR: Lig-Lig','LJ-SR: Lig-Lig','Coul-14: Lig-Lig','LJ-14: Lig-Lig','Coul-SR: Lig-Sol','Coul-SR: Lig-Sol'], write_out = True):
    savefolder_name.mkdir(parents=True, exist_ok=True)
    
    dfs_in_dict_dPCA = e_dataframes_DescPCA_variance.main(variance=variance, include = include, write_out = False)
    ...
    if write_out:
        dataframe_processing.save_dict_with_dfs(dfs_in_dict_merged, save_path=savefolder_name)
